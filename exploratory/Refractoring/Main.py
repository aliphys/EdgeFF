...
parser.add_argument('--rep-epochs', type=int, default=3, help='Number of representation training epochs.')
parser.add_argument('--softmax-epochs', type=int, default=3, help='Number of softmax training epochs.')
...
parser.add_argument('--hw-interval-ms', type=int, default=1000, help='Sampling interval for tegrastats in milliseconds (default 1000).')
...
train_inputs_full = torch.cat([d for d, _ in train_loader], dim=0)
train_targets_full = torch.cat([t for _, t in train_loader], dim=0)
...
# test data
test_inputs_full = torch.cat([d for d, _ in test_loader], dim=0)
test_targets_full = torch.cat([t for _, t in test_loader], dim=0)
...
# validation data
val_inputs_full = torch.cat([d for d, _ in val_loader], dim=0)
val_targets_full = torch.cat([t for _, t in val_loader], dim=0)
