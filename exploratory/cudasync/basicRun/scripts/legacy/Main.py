...
import basicRun.model as Train
import basicRun.evaluation as Evaluation
import basicRun.utils as tools
...
def main():
    ...
    parser.add_argument('--config', type=str, default='configs/run.yaml', help='Path to YAML config file')
    parser.add_argument('--project', type=str, default='edgeff-refactor', help='wandb project name.')
    ...
    current_dir = Path(__file__).resolve().parent  # scripts/legacy directory
    package_root = PACKAGE_ROOT  # basicRun package root
    files_to_add = [
        current_dir / 'Main.py',
        package_root / 'Train.py',
        package_root / 'evaluation.py',
        package_root / 'tools.py',
        package_root / 'tegrats_monitor.py',
    ]
