"""Auxiliary utilities for Forward-Forward network analysis."""

import numpy as np
import torch
from basicRun.evaluation import print_results


def calculate_goodness_distributions(matrix, y_predicted_on_layer, targets, num_layers):
    mean_all = 0
    std_all = 0
...
def analysis_val_set(model, inputs, targets):
    ...
    softmax_output_on_layer = np.zeros((num_layers, num_val_samples, 10))
    for i in range(num_batches):
        ...
        y_predicted_on_layer[:, chunk_indices_validation[i]] = temp_y_predicted_on_layer
        cumulative_goodness_on_layer[:, chunk_indices_validation[i]] = temp_cumulative_goodness_on_layer
        softmax_output_on_layer[:, chunk_indices_validation[i], :] = temp_softmax_output_on_layer
    mean = []
    std = []
    for i in range(num_layers):
        temp_mean, temp_std = calculate_goodness_distributions(
